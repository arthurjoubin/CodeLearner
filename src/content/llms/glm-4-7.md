---
name: "GLM 4.7"
description: "Zhipu AI's flagship model with strong Chinese language capabilities and competitive coding performance. Best value for Chinese-English bilingual tasks."
provider: "Zhipu AI"
contextWindow: "128K tokens"
pricing: "$0.50 per million tokens (input), $1.50 per million tokens (output)"
website: "https://bigmodel.cn"
strengths:
  - "Excellent Chinese-English bilingual capabilities"
  - "Cost-effective pricing"
  - "Strong coding performance"
  - "Good long-context retention"
  - "Open-source availability"
weaknesses:
  - "Less mature ecosystem outside China"
  - "Documentation primarily in Chinese"
  - "Smaller community compared to Western models"
bestFor:
  - "Chinese language tasks"
  - "Bilingual content creation"
  - "Cost-conscious coding projects"
  - "Regional deployment in Asia"
benchmarks:
  - { name: "HumanEval", score: "85%" }
  - { name: "MMLU", score: "81%" }
  - { name: "C-Eval", score: "89%" }
pubDate: "2025-02-01"
open: true
specialty: "Bilingual Chinese-English"
---

## Overview

GLM 4.7 is Zhipu AI's latest flagship model, offering strong performance in both Chinese and English with particular strengths in coding tasks and bilingual applications.

## Key Strengths

- **Bilingual Excellence**: Superior performance in Chinese-English translation and mixed-language tasks
- **Cost Efficiency**: Competitive pricing making it accessible for high-volume applications
- **Coding Capabilities**: Strong performance on programming benchmarks
- **Open Weights**: Available for self-hosting and customization

## Architecture

- **Context Window**: 128K tokens with efficient long-context handling
- **Parameters**: Dense transformer architecture
- **Training**: Extensive Chinese and English corpus

## Pricing

- Input: ~$0.50 per million tokens
- Output: ~$1.50 per million tokens
