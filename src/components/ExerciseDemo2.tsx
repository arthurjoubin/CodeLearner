...
export function ExerciseDemo2({ onComplete }: { onComplete?: () => void }) {
  ...
  const initialSegments = [
    { text: '// Filter positive numbers\n', cls: 'text-gray-500' },
    { text: 'function ', cls: 'text-purple-600' },
    { text: 'filterPositive', cls: 'text-gray-900 font-bold' },
    { text: '(', cls: 'text-gray-800' },
    { text: 'numbers', cls: 'text-gray-800' },
    { text: ') {\n', cls: 'text-gray-800' },
    { text: '  return ', cls: 'text-purple-600' },
    { text: 'numbers', cls: 'text-gray-800' },
    { text: '.', cls: 'text-gray-800' },
    { text: 'fileter', cls: 'text-red-600 font-bold' },
    { text: '(', cls: 'text-gray-800' },
    { text: 'n', cls: 'text-gray-800' },
    { text: ' => ', cls: 'text-gray-800' },
    { text: 'n', cls: 'text-gray-800' },
    { text: ' ', cls: 'text-gray-800' },
    { text: '>', cls: 'text-gray-800' },
  ...
  const fixSegments = [
    { text: '\n\n', cls: '' },
    { text: '// Fix: correct method name\n', cls: 'text-gray-500' },
    { text: 'numbers', cls: 'text-gray-800' },
    { text: '.', cls: 'text-gray-800' },
    { text: 'filter', cls: 'text-purple-600 font-bold' },
    { text: '(n => n > ', cls: 'text-gray-800' },
    { text: '0', cls: 'text-blue-600' },
    { text: ');', cls: 'text-gray-800' },
  ];
  ...
  const hintText = 'Typo detected! "fileter" should be "filter". Method names must be spelled correctly.';
    ...
    switch (phase) {
      case 'typing':
        timer = setTimeout(
          ...
          typedChars < totalInitialChars ? 40 : 1200
        );
        break;
      case 'analyzing':
        timer = setTimeout(() => setPhase('hint'), 1800);
        break;
        case 'hint':
        timer = setTimeout(
      ...
      case 'fixing':
        timer = setTimeout(
          ...
          fixChars < totalFixChars ? 45 : 1200
        );
        break;
      case 'reanalyzing':
        timer = setTimeout(() => setPhase('validated'), 1400);
        break;
      case 'validated':
        timer = setTimeout(() => {
          if (onComplete) {
  ...
  return (
    <div className={`border-2 rounded-lg bg-white max-w-2xl overflow-hidden transition-colors duration-500 ${isValidated ? 'border-purple-400' : 'border-gray-300'}`}>
      <div className="flex items-center gap-2 px-4 py-2.5 border-b border-gray-200 bg-gray-50">
        <div className="w-3 h-3 rounded-full bg-red-400" />
        <div className="w-3 h-3 rounded-full bg-yellow-400" />
        <div className="w-3 h-3 rounded-full bg-purple-400" />
        <span className="ml-2 text-[10px] text-gray-400 uppercase font-bold tracking-wider">script.js</span>
      </div>

      <div className="p-4 md:p-6 min-h-[170px]">
      ...
      <div className="px-4 py-3 border-t border-gray-200 bg-gray-50 min-h-[44px] flex items-center">
        ...
        {phase === 'typing' && typedChars > 0 && (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-gray-500 font-medium">Writing JavaScript...</span>
          </div>
        )}
        {(phase === 'analyzing' || phase === 'reanalyzing') && (
          <div className="flex items-center gap-2">
            <div className="w-4 h-4 border-2 border-purple-500 border-t-transparent rounded-full animate-spin" />
            <span className="text-xs text-purple-600 font-bold">AI is checking your code...</span>
          </div>
        )}
        {phase === 'hint' && (
        ...
        {phase === 'validated' && (
          <div className="flex items-center gap-2 animate-pop">
            ...
            <span className="text-xs text-purple-700 font-bold">Code validated! +125 XP earned</span>
          </div>
        )}
      </div>
    </div>
